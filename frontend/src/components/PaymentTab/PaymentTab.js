// frontend/src/components/PaymentTab/PaymentTab.js
...
export default class PaymentTab extends React.Component {
  ...
  renderSeatNumbers = () => {
    ...
    let seatArr;
    try {
      seatArr = JSON.parse(seatArray);
      if (!Array.isArray(seatArr)) seatArr = [seatArr];
    } catch (e) {
      seatArr = [seatArray];
    }

    return seatArr.map((seat, idx) => <p key={idx}>{seat}</p>);
  };
